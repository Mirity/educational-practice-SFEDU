...
export default class ServiceCenterResource {
    ...
    public async getServiceCenterById(id: number): Promise<DbServiceCenter | undefined> {
        const serviceCenters = await Database.makeQuery<DbServiceCenter[]>(`${this.getServiceCentersQuery} where service_center.id = ${id} GROUP BY service_center.id`, null);

        return serviceCenters[0];
    }

    public async addNewServiceCenter(params: ServiceCenter) {
